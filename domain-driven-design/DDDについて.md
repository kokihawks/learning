...
ソフトウェア設計手法には、抽象度の異なる複数の概念があります:

| 種類 | 例 |
|------|-----|
| プログラミングパラダイム | オブジェクト指向（OOP）、関数型プログラミング |
| アーキテクチャスタイル | マイクロサービス、モノリス |
| アーキテクチャパターン | レイヤードアーキテクチャ、ヘキサゴナル |
| デザインパターン | MVC、Repository |

DDDはこれらと排他ではなく**組み合わせて使う思想**です。例えば「DDD × レイヤードアーキテクチャ × OOP」のように併用するのが一般的です。

DDDは、特に複雑なビジネスロジックを扱うシステムにおいて、ビジネスドメインを中心に据えることで独自の価値を提供します。
...
3. **ドメインモデル**: ビジネスの概念とルールを表現する抽象モデルを構築する
4. **モデルと実装の一致**: ドメインモデルがコードに直接反映される
5. **複雑さの管理**: 大きなシステムを境界づけられたコンテキストに分割

### 戦略的設計と戦術的設計

DDDは大きく2つの領域に分かれます:

- **戦略的設計（Strategic Design）**: ドメインをどう分割し、どう整理するかを扱う上流の設計。境界づけられたコンテキスト、ユビキタス言語、ドメインモデルなどが含まれる
- **戦術的設計（Tactical Design）**: ドメインモデルをコードに落とし込むための実装パターン。エンティティ、値オブジェクト、集約、リポジトリなどが含まれる

本ドキュメントでは戦略的設計の基本概念を扱います。学習順序の全体像は [知識の優先度マップ](知識の優先度マップ.md) を参照してください。

## DDDの基本概念

### ビジネスドメインとは

ビジネスドメイン（Business Domain）とは、ソフトウェアが解決しようとしているビジネス領域や知識領域のことを指します。システムが扱う問題や業務の知識領域であり、以下のような具体例があります:
...
→ 詳しくは [ドメインとサブドメイン](01_戦略的設計/説明/ドメインとサブドメイン.md) を参照
...
→ 詳しくは [ユビキタス言語](01_戦略的設計/説明/ユビキタス言語.md) を参照
...
→ 詳しくは [ドメインモデル](01_戦略的設計/説明/ドメインモデル.md) を参照
...
→ 詳しくは [境界づけられたコンテキスト](01_戦略的設計/説明/境界づけられたコンテキスト.md) を参照
